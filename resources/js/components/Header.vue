<template>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <!-- IF -->
        <nav class="hidden md:flex space-x-10"  v-if="isLoggedIn">
            <div class="relative">
                ...
                <button type="button" class="text-gray-500 group bg-white rounded-md inline-flex items-center text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" aria-expanded="false">
                    <router-link class="mr-4" to='/dashboard' exact> <span>Dashboard</span></router-link>
                </button>
            </div>
            <router-link class="text-base font-medium text-gray-500 hover:text-gray-900" to='#'>Books</router-link>
        </nav>

        <!-- Else -->
        <nav class="hidden md:flex space-x-10" v-else>
            <div class="relative">
                <!-- Item active: "text-gray-900", Item inactive: "text-gray-500" -->
                <button type="button" class="text-gray-500 group bg-white rounded-md inline-flex items-center text-base font-medium hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" aria-expanded="false">
                    <router-link class="mr-4" to='/' exact> <span>Home</span></router-link>
                </button>
            </div>

            <router-link class="text-base font-medium text-gray-500 hover:text-gray-900" to='/about'>About</router-link>
            <a href="#" class="text-base font-medium text-gray-500 hover:text-gray-900"> Docs </a>
        </nav>
        <div class="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
             <router-link class="whitespace-nowrap text-base font-medium text-gray-500 hover:text-gray-900" to='/login'>Login</router-link>
